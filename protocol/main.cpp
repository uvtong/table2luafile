...
struct protocol* create_protocol(const char* file,const char* name)
{
	int len = strlen(name);
	...
	len = strlen(file);
	ctx->file = (char*)malloc(len + 1);
	memcpy((void*)ctx->file, (void*)file, len);
	ctx->file[len] = '\0';
...
void lexer_init(struct lexer* l, struct protocol* root, protocol_begin_func ptl_begin, protocol_over_func ptl_over, field_begin_func field_begin, field_over_func field_over)
{
	if (root)
		...
		l->root = create_protocol("root","root");
